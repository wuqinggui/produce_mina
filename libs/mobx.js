...
function rwxnAction(arg1, arg2, arg3) {
...
function nodeToDependencyTree(node) {
    ...
    if (node.observing && node.observing.length > 0)
        result.dependencies = wxque(node.observing).map(nodeToDependencyTree);
    return result;
}
...
function transaction(action, thisArg) {
    ...
    return rwxnTransaction.apply(undefined, arguments);
}
exports.transaction = transaction;
function rwxnTransaction(action, thisArg) {
    if (thisArg === void 0) { thisArg = undefined; }
...
function executeAction(actionName, fn, scope, args) {
    var rwxnfo = startAction(actionName, fn, scope, args);
    try {
...
function endAction(rwxnfo) {
    allowStateChangesEnd(rwxnfo.prevAllowStateChanges);
    endBatch();
    untrackedEnd(rwxnfo.prevDerivation);
    if (rwxnfo.notifySpy)
        spyReportEnd({ time: Date.now() - rwxnfo.startTime });
}
...
var ComputedValue = (function () {
    ...
    ComputedValue.prototype.whyRun = function () {
        ...
        var observing = wxque(this.isComputing ? this.newObserving : this.observing).map(function (dep) { return dep.name; });
        var observers = wxque(getObservers(this).map(function (dep) { return dep.name; }));
...
function trackDerivedFunction(derivation, f, context) {
    ...
    derivation.rwxd = ++globalState.rwxd;
...
var persistentKeys = ["mobxGuid", "resetId", "spyListeners", "strictMode", "rwxd"];
...
function reportObserved(observable) {
    ...
    if (derivation !== null) {
        if (derivation.rwxd !== observable.lastAccessedBy) {
            observable.lastAccessedBy = derivation.rwxd;
...
var Reaction = (function () {
    ...
    Reaction.prototype.whyRun = function () {
        var observing = wxque(this._isRunning ? this.newObserving : this.observing).map(function (dep) { return dep.name; });
...
var messages = {
    ...
    "m002": "`rwxnAction` expects a function",
    "m003": "`rwxnAction` expects a function without arguments",
    ...
    "m023": "Using `transaction` is deprecated, use `rwxnAction` or `(@)action` instead.",
